...
describe WorldMap do
  ...
  let(:temperatore_dropoff) { max_temperature / (height - map_equator) }
  ...
  it 'assigns a temperature value to each tile' do
    y = 1
    expect(world_map.map[1][y].temperature_value).to eq max_temperature - ((map_equator - y).abs * temperature_dropoff_value)
  end
end
